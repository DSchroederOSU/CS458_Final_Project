...
%% Abstract section.
\abstract{This paper is a visualization evaluation of the program Poemage, created by Nina McCurdy, Julie Lein, Katharine Coles, and Miriah Meyer from the University of Utah. We will evaluate the effectiveness of this visualization as well as suggest future implementations that could enhance the application. The evaluation and results include testing foreign languages with the Poemage application, suggestions for future improvements to the UI, and a usability report that highlights what features of Poemage users found usefuland what features users disliked.
} % end of abstract
...
\teaser{
  \centering
  \includegraphics[width=\linewidth]{Header.eps}
...
Lastly, users were presented with our proposed redesign and asked which design they preferred. \break
...
We also used Solo tú by Anónimo to evaluate the effectiveness of Poemage when using a piece that was originally written in Spanish.
...
\begin{figure}[H]
\centering
\includegraphics[width=5cm,height=4cm]{highlight_mock_up.eps}
\caption{A mock up example of the background color text highlighting technique. \label{overflow}}
\label{fig:highlight}
\end{figure}
\indent As shown in \autoref{fig:highlight} above, our implementation will allow for multiple background colors to be partitioned throughout a single word and clearly identify which syllable is being targeted by which filter. We feel as though this technique provides clearer and more precise analysis of the given text and provides user with a better understanding of the sonic devices present.
...
\subsection{Usability Evaluation}
\indent The usability testing confirmed many of our assumptions about the issues with Poemage. All of the participants were able to complete the task, but there were still some issues with the modes and syllables. Users were unable to understand the purpose of the modes option and expressed that it was confusing. They were aware that it is used for formatting, but didn’t know why it would be useful. Since we would like to optimize Poemage for education, we felt the results from Task 1 showed that it would be best to remove the modes option from Poemage to improve usability. Task 1 also showed that users were able to figure out how to select a specific syllable from a sonic rhyme, but they would have liked to know the definition of a “Vowel Slant Rhyme”. This would suggest that it may be beneficial to include definitions of the different rhymes within the Poemage interface so users are able to understand the details of the rhymes they are analyzing. This would also make Poemage more effective for users that might not have a strong background in poetry analysis and close reading.\break   
\indent Task 2 showed that it should be obvious to users that you can scroll below the sonic rhymes in the set view. 2 out of 5 of the participants were unable to discover the visual rhyme section in Poemage and this is most likely due to the lack of cues to suggest to the user that scrolling is possible. This provides support for the arrow proposed in the Enhancements section. All users that were able to navigate to the single character section understood how to select the letter ‘S’ which suggested that the current interface for selecting specific syllables is effective. When presented with our redesign of the interface, many of the participants expressed that highlighting syllables instead of letters makes sense for Poemage. They understood how sounds are highlighted instead of the entire word that contains the rhyme. Therefore, we feel that our proposed redesign was effective. There was little commentary on the improved spacing and removal of the modes section, but highlighting and the arrow were well received. 
...
The Poemage application proved to be a very effective visualization tool for English poetry and text, though when evaluated as a text-parsing tool for other languages, we found problems in the implementation. The UI proved to be effective as well, with only a few minor changes that our group would like to see implemented as mentioned above in \autoref{enhancements} that would improve overall usability and clarity for the user. Our future work consists of changing the text highlighting technique currently implemented as drawn ellipses to changing the background color of the targeted syllables. We feel that this would improve the clarity and readability of the Poemage interface and provide a better visualization of the sonic devices within the text for the user. More future work is mentioned above in \autoref{enhancements} that involve changes to the UI like text spacing and a scrollable indication arrow in the filters view window as well as on-hover capability to describe the rhyme rules that are implemented within Poemage.\\ 
\indent Overall, our usability results concluded that Poemage is a well-developed, effective tool for the Purpose the original design team had in mind, but there is room for improvement to accomodate novice users and expanding the potential of the tool itself by incorporating language other than English. As an educational tool for visualizing the sonic topography of an English poem, Poeamge performs substantially well, but our team would like to see improvements to expand the project to a broader audience and add support for more functionality.
\acknowledgments{
