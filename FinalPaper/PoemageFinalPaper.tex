...
\indent The path view in Poemage utilized multiple algorithms for rendering the different paths amongst the poem's nodes. The program uses a shortest path algorithm for creating a line that connects multiple nodes in the path view. There are also path rendering algorithms in place that handle multiple paths being drawn, rerouting techniques to avoid path intersecting isolated nodes, and to fill the intersecting paths with a shaded region to show ``mergence, divergence, and emergence'' \cite{poemage}.\\
\indent Poemage uses strung-together conditionals to parse each word and analyze the sounds present.
